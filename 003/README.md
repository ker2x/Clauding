...
- **003 (SAC)**: Continuous action space (infinite actions), actor-critic learning

### Action Space
- **002**: Discretized into 9 actions (3 steering × 3 gas/brake)
- **003**: Native continuous actions `[steering ∈ [-1,1], gas ∈ [0,1], brake ∈ [0,1]]`

### State Modes
- **002**: "snapshot" mode (36D vector), "vector" mode (11D, too limited), "visual" mode
- **003**: "vector" mode (36D, renamed from snapshot), "visual" mode (11D mode removed)

### Exploration
- **002**: ε-greedy (epsilon decays linearly)
- **003**: Maximum entropy (automatic entropy tuning via α parameter)

### Network Updates
- **002**: Single Q-network with target network
- **003**: Twin Q-networks + actor network + automatic alpha tuning

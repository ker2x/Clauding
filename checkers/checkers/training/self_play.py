...
class SelfPlayGame:
    ...
    def play_game(
        ...
        Args:
            on_step: Optional callback function called after every move.
                    Signature: (game_state_array, policy_dist, move_count, current_player, legal_moves)

        Returns:
            (states, policies, values): Training examples from the game
                - states: List of game states (8, 10, 10)
                - policies: List of MCTS policy distributions (150,)
        ...
        # Call on_step for initial state
        if on_step:
            state = game.to_neural_input()
            # Pass empty policy for initial state or handle it
            on_step(state, None, move_count, game.current_player, game.get_legal_moves())
        ...
        while not game.is_terminal() and move_count < max_moves:
            ...
            if action < len(legal_moves):
                ...
                # Call callback
                if on_step:
                    on_step(game.to_neural_input(), policy, move_count, game.current_player, game.get_legal_moves())
            else:
...
def play_games_parallel(
    ...
    """
    Play multiple games sequentially (parallel version would use multiprocessing).
    ...
    for game_idx in range(num_games):
        states, policies, values = self_play.play_game(on_step=on_step)
...
if __name__ == "__main__":
    ...
    # Play multiple games
    print("\nPlaying 5 games...")
    all_states, all_policies, all_values = play_games_parallel(
        network, Config, device, num_games=5
    )

    print(f"\nTotal training examples: {len(all_states)}")
    print(f"Average game length: {len(all_states) / 5:.1f} moves")
